...
class Node:
    ...
    def get_exec_id_type(self):
        # or all(not value for value in self.exec_identifier.values())
        if len(list(self.exec_identifier.keys()))==0:
            return "xPath"
        else:

            if 'text' in self.exec_identifier and self.exec_identifier['text']:
                text_value = self.exec_identifier.pop('text')
                self.exec_identifier = {'text': text_value, **self.exec_identifier}

            i = 0
            while i < len(list(self.exec_identifier.keys())):
                if self.exec_identifier[list(self.exec_identifier.keys())[i]] != '':
                    break
                if i == len(list(self.exec_identifier.keys()))-1:
                    break
                if list(self.exec_identifier.keys())[i]=="xPath":
                    i+=1
                    continue
                i += 1


            return list(self.exec_identifier.keys())[i]
    ...
    def get_exec_id_val(self):
        ...
        if len(list(self.exec_identifier.keys())) == 0:
            ...
            # print("self.exec_identifier.item",self.exec_identifier.items())
            # 假设索引i
            ...
            while i < len(list(self.exec_identifier.keys())):
                ...
                if i == len(list(self.exec_identifier.keys()))-1:
                    break
                i += 1
            return self.exec_identifier[list(self.exec_identifier.keys())[i]]
    ...
    def get_middle_point(self):
        ...
        brx = float(points[1].split(",")[0])
        bry = float(points[1].split(",")[1][:-1])
        x = (tlx + brx) // 2
        y = (tly + bry) // 2
        return [x, y]
