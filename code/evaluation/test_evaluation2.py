...
sys.path.insert(0, os.path.join(current_dir_path, '..', 'dynamic_generation'))
...
def find_linear_states_and_triggers(linear_model):
    ...
    # print("linear_model.states",linear_model.states)
    # for key, value in vars(linear_model).items():
    #     print(key, value)
...
def eval_AUT_per_usage(appname, usage_name,functionality): # appname: etsy, usage_name: 1-SignIn
    ...
    if appname in ['sudoku', 'reversi', 'andttt', 'chess']:
        eval_json_file = open(os.path.join(USAGE_REPO_ROOT_DIR, 'usage_data', usage_name, appname+'-'+functionality,'eval_results.json'), 'r')
        eval_json0 = json.load(eval_json_file)
        tests = {}
        for key in eval_json0:
            prefix = key.split('-')[0]  # Extract the prefix (e.g., '0', '1', '2', '3')
            if prefix not in tests:
                tests[prefix] = {'states': [], 'transitions': []}

            # Append state and transition to the appropriate test based on prefix
            tests[prefix]['states'].append(eval_json0[key]['true_screen_IR'])
            tests[prefix]['transitions'].append(eval_json0[key]['true_widget_IR'])


        # Print each test
        for prefix, test in tests.items():
            print(f"test{prefix}:", test)

        # Generate the list of cleaned tests
        human_test_list = [clean_test(test) for test in tests.values()]

    else:
        for linear_model_path in glob.glob(os.path.join(USAGE_REPO_ROOT_DIR, 'usage_data', usage_name, appname+'*', 'linear_model.pickle')):
  
            print("linear_model_path",linear_model_path)
...
def eval_test_pairs(appname, usage_name, human_test_list, generated_test_list):
    ...
    for test_id in range(len(generated_test_list)):
        for human_video_id in range(len(human_test_list)):
            ...
            print("reduction_list",reduction_list)
...
if __name__ == '__main__':
    functionality = 'terms'
    usage_name = usage_folder_map[functionality]
    # usage_name = usage_folder_map['textsize']
    # print("usage_name",usage_name)
    eval_usage_batch(usage_name,functionality)
